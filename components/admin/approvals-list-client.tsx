...
import { approveCertificateAction, rejectCertificateAction } from "@/app/admin/actions";
...
export function ApprovalsListClient({ pendingCerts, approvedCerts }: ApprovalsListClientProps) {
  ...
  async function handleMint(cert: Certificate) {
    ...
    try {
      // call the Soroban mint transaction
      ...
      // Save to Supabase (updating the existing record)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-[#EAD8CF] pb-px">
        ...
        >
          Pending Admin Review ({pendingCerts.length})
        </button>
        <button
      ...
      {activeTab === "pending" ? (
        pendingCerts.length > 0 ? (
                      ...
                      {rejectingId === cert.id ? (
                        <div className="mt-2 space-y-2">
                          ...
                          <div className="flex gap-2">
                            <form action={rejectCertificateAction}>
          ...
          <div className="rounded-xl border border-[#E9D6CD] bg-[#FFF8F4] p-8 text-center text-sm text-[#6B5A54]">
            No certificates pending Admin approval.
          </div>
        )
